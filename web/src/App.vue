  ...
  if (itemData?.type) {
    switch (itemData.type) {
      ...
      case 'head2head':
        ...
        break;
case 'updateTablet':
    if (globalStore.baseData?.data && itemData?.data) {
        globalStore.baseData.data.racerNames = itemData.data
        if (itemData.data[0]) {
            globalStore.baseData.data.currentRacerName = itemData.data[0].racername
        }
    }
    break;
      default:
