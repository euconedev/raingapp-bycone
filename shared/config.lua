...
local frameworkState = GetResourceState('vrp')
local frameworkStateEx = GetResourceState('vrpex')
local frameworkStateQB = GetResourceState('qb-core')
local frameworkStateESX = GetResourceState('es_extended')

-- Forçar o uso do framework vRP
Config.Framework = 'vrp'

-- Verificação de debug para confirmar o framework
print('DEBUG: Framework selecionado: ' .. Config.Framework)
print('DEBUG: vRP resource state: ' .. frameworkState)
print('DEBUG: vRPex resource state: ' .. frameworkStateEx)

Config.Debug = SetGpsFlashes
Config.EnableCommands = false

Config.Locale = TranslationsPTBR -- This must match one of the variables in your locales/x.lua
...
Config.BasePermission = 'Admin' -- this is the base user type your racingapp users can create if you allow creation in app. Should match the user you want from Config.Permissions
...
-- Configuração universal de jobs - funciona em qualquer framework
Config.AllowedJobs = { 
    ['tuner'] = { racer = 1, creator = 4, master = 5, god = 5 },
    ['spongebob'] = { racer = 1 },
    -- Para vRP/vRPex, use os nomes dos grupos como estão no seu servidor
    ['police'] = { racer = 1, creator = 3, master = 4, god = 5 },
    ['mechanic'] = { racer = 1, creator = 2, master = 3, god = 4 },
}
...
-- Sistema de pagamentos universal
Config.Payments = {
...
Config.AutomatedOptions = {
    ...
    minimumParticipants = 10,      -- The least amount of participants that are needed for the race to start
    timeToWaitForPlayers = 30 * 1000, -- 30 seconds
...
-- DEBUG: Mostra qual framework foi detectado
if Config.Debug then
    print('^2[UNIVERSAL BRIDGE]^7 Framework detectado: ' .. Config.Framework)
end