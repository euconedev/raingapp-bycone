...
-- Coloque isso junto com outras configurações no início do arquivo

-- Configuração do Webhook
local DiscordWebhook = {
    enabled = true,
    url = "https://discord.com/api/webhooks/1433477717986246778/njH7wfjWXBgt8SlksEYv2wiPLSj4Br-E1vvntHjAqy7NY6nm-n3zp-r4sHnQtJBfx3LC", -- COLE SUA URL AQUI
    botName = "Racing System",
    botAvatar = "https://media.discordapp.net/attachments/1414206752286838884/1414209086475730994/a_ca5acafb919a01689338fe26c1603d1d.gif?ex=69049bd8&is=69034a58&hm=5070b1e79050ecac2a920e3d9eae74c034ba2e02fe7765acc1f0283dfbb8b6d0&=&width=856&height=856"
}
...
RegisterNUICallback('UiGetRacingResults', function(data, cb)
    local result = cwCallback.await('cw-racingapp:server:getRaceResults', data.identifier)
...
RegisterNUICallback('UiQuickHost', function(track, cb)
    ...
    DebugLog('UiQuickHost: track object received', json.encode(track, { indent = true }))
...
RegisterNUICallback('UiCreateUser', function(data, cb)
    ...
    if data.racerName and data.selectedAuth then
        local success = AttemptCreateUser(data.racerName, data.racerId, data.selectedAuth.fobType, data.selectedAuth.purchaseType)
        cb(success)
    else
...
RegisterNUICallback('UiSendInvite', function(data, cb)
    ...
    if CurrentCrew == nil then
        NotifyHandler(Lang("not_in_crew"), 'error')
        cb(false)
        return
    end
