    ...
    if result then
        if RacingCrews[crewName] then
            for _, member in pairs(RacingCrews[crewName].members) do
                local playerSrc = Racing.Functions.GetSrcOfPlayerByCitizenId(member.citizenID)
                if playerSrc ~= nil then
                    notifyPlayer(playerSrc, Lang("disbanded_crew"), 'error')
                end
            end
...
local function inviteToCrew(invitedBySource, invitedCitizenId, crewName)
    if crewName == nil then
        if useDebug then print('DEBUG: crewName is nil in inviteToCrew. Aborting invite.') end
        return false
    end
    local playerSrc = Racing.Functions.GetSrcOfPlayerByCitizenId(invitedCitizenId)
    if playerSrc ~= nil then
        if getRacingCrewThatCitizenIDIsIn(invitedCitizenId) then
            notifyPlayer(invitedBySource, Lang("racer_already_in_crew"), 'error')
            return false
        end
        ...
        notifyPlayer(playerSrc, Lang("pending_crew_invite") .. ' ' .. crewName)
        return true
    else
...
local function acceptInvite(racerName, invitedCitizenId)
    ...
    if crewName then
        ...
        if ActiveInvites[invitedCitizenId].invitedBySource then
            ...
            notifyPlayer(ActiveInvites[invitedCitizenId].invitedBySource,
...
RegisterServerCallback('cw-racingapp:server:sendInviteClosest',
    function(source, invitedBySource, invitedSource, crewName)
        local citizenId = Racing.Functions.GetCitizenId(invitedSource)
        if not citizenId then return notifyPlayer(source, Lang("person_no_exist"),
                'error') end
...
RegisterServerCallback('cw-racingapp:server:createCrew', function(source, founderName, founderCitizenId, crewName)
    ...
    if RacingCrews[trimmedCrewName] then
        notifyPlayer(source, Lang("name_taken"), 'error')
        return false
    end
    if canCreateCrew then
        ...
        notifyPlayer(source, Lang("disband_crew_first"), 'error')
    end
    return false
end)
...
RegisterServerCallback('cw-racingapp:server:leaveCrew', function(source, memberName, citizenId, crewName)
    ...
    if isFounder then
        notifyPlayer(source, Lang("founder_can_not_leave"), 'error')
    end
...
RegisterServerCallback('cw-racingapp:server:kickMemberFromCrew', function(source, memberName, citizenId, crewName)
    local userCurrentSource = Racing.Functions.GetSrcOfPlayerByCitizenId(citizenId)
    if not RacingCrews[crewName] then
        if useDebug then print('The racing crew did not exist') end
